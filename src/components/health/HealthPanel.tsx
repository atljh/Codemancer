...
import { X, ChevronDown, ChevronRight, FileText, AlertTriangle, Bug } from "lucide-react";
...
export function HealthPanel() {
  ...
  const [expandedSection, setExpandedSection] = useState<DrillSection | null>(null);
  ...
  const handleAxisClick = (axis: string) => {
    setExpandedSection(expandedSection === axis ? null : (axis as DrillSection));
  };
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin min-h-0">
              {loading ? (
                <div className="flex items-center justify-center py-16">
                  ...
                  <span className="ml-3 text-xs text-theme-text-dim font-mono">{t("health.scanning")}</span>
                </div>
              ) : !data ? (
                <div className="text-center text-theme-text-dim text-xs py-16">{t("health.error")}</div>
              ) : (
                <>
                  {/* Radar chart */}
                  <div className="px-4 py-4">
                    <RadarChart scores={data.scores} onAxisClick={handleAxisClick} />
                  </div>

                  {/* Drill-down sections */}
                  <div className="px-4 pb-4 space-y-1">
                    {(["complexity", "coverage", "cleanliness", "file_size"] as DrillSection[]).map((section) => {
                      const isExpanded = expandedSection === section;
                      return (
                        <div key={section} className="glass-panel rounded overflow-hidden">
                          <button
                          ...
                          >
                            ...
                            {sectionIcon(section)}
                            <span className="uppercase tracking-wider">{sectionLabel(section)}</span>
                            <span className="ml-auto text-theme-accent font-bold">{data.scores[section]}</span>
                          </button>
                                ...
                                <div className="px-3 pb-2 space-y-0.5 max-h-48 overflow-y-auto scrollbar-thin">
                                  {section === "complexity" &&
                                    (data.complex_functions.length === 0 ? (
                                      <p className="text-[10px] text-theme-text-dimmer py-1">{t("health.noIssues")}</p>
                                    ) : (
                                      data.complex_functions.map((f, i) => (
                                        ...
                                        >
                                          <span className="text-theme-accent">{f.name}</span>
                                          <span className="text-theme-text-dimmer truncate">{f.file}</span>
                                          <span className="ml-auto text-theme-status-warning shrink-0">{f.lines} {t("health.lines")}</span>
                                        </button>
                                      ))
                                    ))}
                                  ...
                                  {section === "cleanliness" &&
                                    (data.anomalies.length === 0 ? (
                                      ...
                                      data.anomalies.map((a, i) => (
                                        <button
                                          key={i}
                                          onClick={() => handleFileClick(a.file)}
                                          className="w-full text-left text-[10px] font-mono py-0.5 px-1 rounded hover:bg-white/5 text-theme-text-dim flex items-center gap-2"
                                        >
                                          <span className="text-theme-status-warning font-bold">{a.tag}</span>
                                          <span className="truncate">{a.text || a.file}</span>
                                          <span className="ml-auto text-theme-text-dimmer shrink-0">:{a.line}</span>
                                        </button>
                                      ))
                                    ))}
                                    ...
                                    (data.large_files.length === 0 ? (
                                      ...
                                      data.large_files.map((f) => (
                                        ...
                                        >
                                          ...
                                          <span className="ml-auto text-theme-status-error shrink-0">{f.lines} {t("health.lines")}</span>
                                        </button>
                                      ))
                                    ))}
                                </div>
